...
char *CRC_Verify(RegionCRC *region, DWORD_PTR *Size) {
	...
	if (modified > 0) {
		char *mptr = ret = (char *)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, ((sizeof(DWORD_PTR) * 2) * modified) + 1);
...
//	pusha
	...
	ret = callret;
	*cleanup = _cleanup;
	*RegionRet = (DWORD_PTR)CRC_Verify(RegionVerify, RegionRetSize);
