...
static void
CopySections(const unsigned char *data, PIMAGE_NT_HEADERS old_headers, PMEMORYMODULE module, tAddRegion regadd)
{
	int i, size;
	...
	for (i=0; i<module->headers->FileHeader.NumberOfSections; i++, section++) {
		if (section->SizeOfRawData == 0) {
			...
			if (size > 0) {
				...
				section->Misc.PhysicalAddress = (DWORD_PTR)dest;
				memset(dest, 0, size);
				//regadd((DWORD_PTR)dest, (DWORD_PTR)size);
			}
		...
		regadd((DWORD_PTR)((char *)dest), (DWORD_PTR)section->SizeOfRawData);
		section->Misc.PhysicalAddress = (DWORD_PTR)dest;
	}
}
...
FinalizeSections(PMEMORYMODULE module)
{
	...
	PIMAGE_SECTION_HEADER section = IMAGE_FIRST_SECTION(module->headers);
#ifdef _WIN64
	DWORD_PTR imageOffset = (module->headers->OptionalHeader.ImageBase & 0xffffffff00000000);
#else
	...
	for (i=0; i<module->headers->FileHeader.NumberOfSections; i++, section++) {
		...
		if (section->Characteristics & IMAGE_SCN_MEM_DISCARDABLE) {
			...
			VirtualFree((LPVOID)((DWORD_PTR)section->Misc.PhysicalAddress | imageOffset), section->SizeOfRawData, MEM_DECOMMIT);
			continue;
		}
		...
		if (size > 0) {
			...
			if (VirtualProtect((LPVOID)((DWORD_PTR)section->Misc.PhysicalAddress | imageOffset), size, protect, &oldProtect) == 0) {
#ifdef DEBUG_OUTPUT
...
SIZE_T MemorySizeNeeded(const void *data) {
	...
	return (SIZE_T)old_header->OptionalHeader.SizeOfImage;
}
...
HMEMORYMODULE MemoryLoadLibrary(const void *data, void *force_base, DWORD_PTR ImportReplace, int exec, unsigned char **code_addr, void **func, tAddRegion addreg)
{
	...
	if (force_base) {
		code = headers = (unsigned char *)force_base;
		/*
		code = (unsigned char *)VirtualAlloc((LPVOID)force_base,
	...
	*/
	...
	// update position
	result->headers->OptionalHeader.ImageBase = (DWORD_PTR)code;

	// copy sections from DLL file block to new memory location
	CopySections((const unsigned char *)data, old_header, result, addreg);
	...
	if (ImportReplace != 0) {
		typedef int (*tImp)(PMEMORYMODULE);
		tImp fImp = NULL;
		fImp = (tImp)ImportReplace;
		if (fImp(result) != -1)
			replaced = 1;
	}
